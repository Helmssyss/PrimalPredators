...
void UAN_FootprintL::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation){
	...
	if (ABasePredator* Predator = Cast<ABasePredator>(MeshComp->GetOwner())){
		...
		if(MeshComp->GetWorld()->LineTraceSingleByChannel(HitResult,Start,End,ECollisionChannel::ECC_Visibility,QueryParams)){
			...
			MeshComp->GetWorld()->SpawnActor<APredatorFootprint>(Predator->GetLeftFootprintClass(), SpawnFootprintTransform, SpawnParameters);
		}
	}
}
